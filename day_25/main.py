...
same process, you can determine that the door's loop size is 11, because
...
(Transforming the subject number of 5764801 (the card's public key) with
a loop size of 11 (the door's loop size) produces the same encryption
key: 14897079.)
...
def decrypt(card_pub: int, subject: int, mod: int) -> int:
    """Iterates the cryptographic hash until it matches public key"""
    value = 1
    loop_size = 0
...
def encrypt(subject: int, rounds: int, mod: int) -> int:
    """Iterates the cryptographic hash"""
    value = 1
...
def part1() -> None:
    ...
    with open('input.txt') as infile:
        card_pub, door_pub = (int(line)
                              for line in infile.read().splitlines())

    mod = 20201227
    card_priv = decrypt(card_pub, subject=7, mod=mod)
    door_priv = decrypt(door_pub, subject=7, mod=mod)

    print(encrypt(door_pub, rounds=card_priv, mod=mod))
    print(encrypt(card_pub, rounds=door_priv, mod=mod))
