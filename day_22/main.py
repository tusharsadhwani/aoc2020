...
like to play a game of Combat, and there's even an opponent available: a
small crab that climbed aboard your raft before you left.
...
Recursive Combat. (For example, if player 1 draws the 3 card, and player
2 draws the 7 card, this would occur if player 1 has at least 3 cards
left and player 2 has at least 7 cards left, not counting the 3 and 7
...
form the sub-game. (For example, if player 1 drew the 3 card, their deck
in the sub-game would be copies of the next three cards in their deck.)

Here is a complete example of gameplay, where Game 1 is the primary game
of Recursive Combat:
...
def parse_input() -> Tuple[Deque[int], Deque[int]]:
    """Parses decks of player 1 and 2"""
    with open('input.txt') as infile:
        player1_data, player2_data = infile.read().split('\n\n')

    player1_deck = [int(card) for card in player1_data.splitlines()[1:]]
    player2_deck = [int(card) for card in player2_data.splitlines()[1:]]

    player1_deque = deque(player1_deck)
    player2_deque = deque(player2_deck)

    return player1_deque, player2_deque


def part1() -> None:
    """Solution for part 1"""
    player1_deck, player2_deck = parse_input()
    ...
    winning_player = player1_deck or player2_deck

    score = 0
    card_count = len(winning_player)
    for index, card in enumerate(winning_player):
        score += card * (card_count-index)

    print(score)


def combat(
        player1_deck: Deque[int],
        player2_deck: Deque[int]) -> Tuple[Deque[int], Deque[int]]:
    """Plays a game of Recursive Combat."""

    previous_decks: Set[Tuple[Tuple[int, ...], Tuple[int, ...]]] = set()

    while player1_deck and player2_deck:
        decks = (tuple(player1_deck), tuple(player2_deck))
        if decks in previous_decks:
            # simulate player1 win by returning empty deque for player2
            return player1_deck, deque()

        previous_decks.add(decks)

        player1_card = player1_deck.popleft()
        player2_card = player2_deck.popleft()

        if len(player1_deck) < player1_card or len(player2_deck) < player2_card:
            if player1_card > player2_card:
                player1_deck.append(player1_card)
                player1_deck.append(player2_card)
            else:
            ...
            # recursion time
            new_p1_deck = deque(list(player1_deck)[:player1_card])
            new_p2_deck = deque(list(player2_deck)[:player2_card])
            new_p1_deck, new_p2_deck = combat(new_p1_deck, new_p2_deck)

            winner = new_p1_deck or new_p2_deck
            if winner is new_p1_deck:
                player1_deck.append(player1_card)
                player1_deck.append(player2_card)
            else:
                player2_deck.append(player2_card)
...
def part2() -> None:
    ...
    player1_deck, player2_deck = combat(player1_deck, player2_deck)
    winning_player = player1_deck or player2_deck
