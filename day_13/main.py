...
def parse_input() -> Tuple[int, List[int]]:
    """Parses input to return arrival time and bus timings"""
    with open('input.txt') as infile:
        arrival_time = int(infile.readline())
        bus_timings = [int(x)
                       for x in infile.readline().split(',')
                       if x != 'x']

    return arrival_time, bus_timings


def part1() -> None:
    """Solution for part 1"""
    arrival_time, bus_timings = parse_input()

    waiting_times = [x - (arrival_time % x) for x in bus_timings]
    min_waiting_time = waiting_times[0]
    min_waiting_index = 0
    for index, time in enumerate(waiting_times):
        if min_waiting_time > time:
            min_waiting_time = time
            min_waiting_index = index

    bus_id = bus_timings[min_waiting_index]
    print(bus_id * min_waiting_time)
...
def part2() -> None:
    ...
    with open('input.txt') as infile:
        infile.readline()
        bus_timings = [int(x) if x != 'x' else 0
                       for x in infile.readline().split(',')]

    step = 1
    timestamp = 1
    for index, timing in enumerate(bus_timings):
        if timing == 0:
            continue

        while (timestamp + index) % timing != 0:
            timestamp += step
        step *= timing
